...
proc jesd204_validate {{quiet false}} {
  ...
  if {$device_family != "Arria 10" && $device_family != "Cyclone 10 GX" && $device_family != "Stratix 10" && $device_family != "Agilex 7"} {
    if {!$quiet} {
      send_message error "Only Arria 10/Cyclone 10 GX/Stratix 10/Agilex 7 are supported in adi_jesd204_hw.tcl:281."
    }
    return false
  }
...
proc jesd204_compose {} {
  ...
  ## Unsupported device
    send_message error "Only Arria 10/Cyclone 10 GX/Stratix 10/Agilex 7 are supported in adi_jesd204_hw.tcl:429."
  }
